...
class TestTokenizer(unittest.TestCase):
    def test_tokenize(self):
        from nlpds.submission.ex1.word2vec.data.tokenizer import Tokenizer
...
class TestDataset(unittest.TestCase):
    ...
    @staticmethod
    def get_dataset():
        from nlpds.submission.ex1.word2vec.data.dataset import Dataset
    ...
    def test_sentence_to_samples(self):
        ...
        samples = Dataset.sentence_to_samples(
            DummyTokenizedSentence([0, 1, 2]), 1, np.ones(3)
        )
        self.assertEqual(
    ...
    def test_dataset_samples(self):
        ...
        sample_set = {tuple(sample) for sample in dataset}
        self.assertEqual(
            sample_set,
            {
                (0, 1),
