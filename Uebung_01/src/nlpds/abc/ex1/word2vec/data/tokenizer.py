...
class TokenizedSentenceABC(ABC):
    """
    ...
    @property
    @abstractmethod
    def input_ids(self) -> NDArray:
        """Get the input ids of this tokenized sentence as a numpy array."""
        raise NotImplementedError(
            f"{self.__class__.__name__}.input_ids property getter is not implemented"
        )

    @abstractmethod
    def __len__(self) -> int:
        """Get the number of context windows in this tokenized sentence."""
        raise NotImplementedError(
...
class TokenizerABC(ABC):
    ...
    def __call__(self, sentence: str) -> TokenizedSentenceABC:
